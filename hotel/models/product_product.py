...
class ProductProduct(models.Model):
    ...
    def _search(self, args, offset=0, limit=None, order=None, count=False, access_rights_uid=None):
        ...
        if isinstance(checkin_date, str):
            checkin_date = datetime.strptime(context.get('checkin_date'), tools.DEFAULT_SERVER_DATETIME_FORMAT)
        if isinstance(checkout_date, str):   
            checkout_date = datetime.strptime(context.get('checkout_date'), tools.DEFAULT_SERVER_DATETIME_FORMAT)
